...
## 2026-09-23 언더마인 문서 반영 및 구현 체크리스트

공통 `UNDERMINE_GAME_SPEC.md`와 API/Realtime 계약을 동기화했다. 이 절의
항목은 구현 계획이며 현재 완료 상태가 아니다. 공통 문서가 존재한다는 이유로
서버가 `UNDERMINE`을 지원한다고 표시하거나 카탈로그에 활성화하지 않는다.

- [ ] 카탈로그 및 `gameType = UNDERMINE`, 빈 config, 3~10명 검증
- [ ] `game/undermine` Runtime·Service·Projection을 다른 게임과 독립 구성
- [ ] 3라운드 역할 구성·셔플, 개인 손패·드로우·버림 덱과 턴 초기화
- [ ] 길 카드의 0°/180° 회전별 통로 방향 변환
- [ ] `x >= 0`인 모든 빈 좌표에 대해 인접 면 통로 일치와 출발점 연결을 검증해
  `cardOptions.placements` 생성
- [ ] 요청 좌표·회전이 최신 서버 후보에 포함되는지 행동 처리 시 재검증
- [ ] 장비 하나 이상 고장 시 길 배치 차단, 고장 대상 검증과 최초 고장 장비 수리
- [ ] 지도 결과 개인 공개, 파괴 후보, 목적지 공개·금 도달 판정
- [ ] 제한 버리기, 카드 드로우, 턴 전환과 행동 `Idempotency-Key` 처리
- [ ] 라운드 승패, 진영별 금 지급, 다음 라운드 초기화와 3라운드 최종 순위
- [ ] 역할·손패·지도·금 개인화와 재접속 `/state` 복구
- [ ] 규칙 단위 테스트와 HTTP/WebSocket 통합 테스트

길 배치 후보는 Backend가 소유한다. Frontend나 Frontend Mock의 단순 좌표 생성
방식을 서버 구현에 복사하지 않는다. 특히 한 줄로 오른쪽 좌표만 증가시키거나,
`x < 0` 좌표·인접 통로 불일치·출발점에서 분리된 카드·고장 Player의 길
카드를 후보로 반환하는 구현은 계약 위반이다.

UNDERMINE은 Runtime 전용 상태이며 DB/Flyway 변경이 없다.
...
- [x] 빽도의 실제 진입 경로 역행, `OUTER_1 → OUTER_20`,
  `OUTER_20 → OUTER_19 또는 CENTER_9`, 도착 칸 업기/잡기 판정.
