...
투표 종료 후에는 후보별 최종 득표수만 전달한다. 개별 투표 대상은 항상 비공개다.
...
방장의 연결이 일시적으로 종료되어도 즉시 방장 권한을 변경하지 않는다.

정해진 재접속 시간을 초과하거나 명시적으로 Room을 나간 경우 Room 정책에
따라 방장을 변경하고 변경 사실을 참가자에게 전달한다.
